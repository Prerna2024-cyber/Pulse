...
// The calendar date, in the market's own timezone, of the session the worker
// is polling for — 'YYYY-MM-DD'. What lib/freshness.js compares a provider's
// own quote date against, to catch the holidays this schedule doesn't know
// about.
//
// Backend-only, and deliberately not mirrored in frontend/src/marketHours.js:
// nothing in the UI asks this question, and an unused copy over there would be
// one more thing for the drift sweep to have to keep honest.
//
// Derived from sessionBounds rather than recomputed, so "which session"
// carries exactly the meaning it does everywhere else — including through the
// post-close grace window, where this is still the date of the session that
// just ended rather than the next one. That is what lets the settled close be
// stored after the bell while a holiday's stale quote is still rejected.
export function sessionDateForMarket(market, at = new Date()) {
  const schedule = SCHEDULES[market];
  if (!schedule) return null;

  const bounds = sessionBounds(market, at);
  if (bounds === null) return null;

  const clock = wallClockIn(schedule.timeZone, bounds.start);
  if (clock === null) return null;
