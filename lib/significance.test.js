...
// The walkthrough tells new users which moves get flagged, quoting these two
// numbers. frontend/ can't import this file (Vite's root stops at it), so the
// copy there is a duplicate — and a duplicate nothing checks would eventually
// teach a rule the product no longer follows.
test('the UI states the same thresholds the detector applies', async () => {
  const ui = await import('../frontend/src/significanceCopy.js');
  assert.equal(ui.PRICE_THRESHOLD_PERCENT, DEFAULT_PRICE_THRESHOLD_PERCENT);
  assert.equal(ui.VOLUME_MULTIPLIER, DEFAULT_VOLUME_MULTIPLIER);
  ...
  assert.match(ui.SIGNIFICANCE_SENTENCE, /2% or more/);
